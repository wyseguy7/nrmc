...
def write_wait_sequence(filepath, overwrite=False, truncate=0):
    print(filepath)
    folder, filename = os.path.split(filepath)

    out_path = os.path.join(folder, 'involution_wait_sequence.json')
    ...
    for move in process.state.move_log:
        ...
        if move is None:
            wait_sequence.append(counter)
            counter = 0

    with open(out_path, mode='w') as f:
        json.dump(wait_sequence, f)


func = functools.partial(write_wait_sequence, overwrite=overwrite, truncate=args.truncate)
