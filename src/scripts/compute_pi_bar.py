...
def compute_pibar(filepath_csv, states=6, overwrite=False):

    df_filepaths = pd.read_csv(filepath_csv)
    fo, fi = os.path.split(filepath_csv)
    out_path = os.path.join(fo, 'pibar_'+fi)
    ...
    df_out = pd.DataFrame(pibar_total)
...
func = functools.partial(compute_pibar, overwrite=overwrite)
